...
export interface EditorDocumentToolsView {
  readonly snapshot: EditorDocumentToolsState;
  subscribe(listener: (state: EditorDocumentToolsState) => void): () => void;
  dispose(): void;
...
export interface EditorDocumentToolsPageReference {
  readonly kind: "editorDocumentTools";
  connect(): Promise<EditorDocumentToolsView>;
}
const pageEditorDocumentToolsReferences = new Map<string, WeakRef<EditorDocumentToolsPageReference>>();
const pageEditorDocumentToolsFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorDocumentToolsPageReference> }>(entry => {
  if (pageEditorDocumentToolsReferences.get(entry.id) === entry.reference) pageEditorDocumentToolsReferences.delete(entry.id);
});
export function pageEditorDocumentTools(id: string): EditorDocumentToolsPageReference {
  let reference = pageEditorDocumentToolsReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorDocumentTools", connect: () => connectEditorDocumentToolsAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorDocumentToolsReferences.set(id, weak);
  pageEditorDocumentToolsFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
const bridgeContract = "Runic.Translations.Editor.EditorDocumentToolsViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorDocumentTools(): Promise<EditorDocumentToolsView> { return connectEditorDocumentToolsAt("editorDocumentTools"); }
async function connectEditorDocumentToolsAt(route: string, needsMount = false): Promise<EditorDocumentToolsView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorDocumentToolsState {
    ...
    return state as EditorDocumentToolsState;
  }
  async function invoke(name: string, ...args: unknown[]): Promise<EditorDocumentToolsState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorDocumentToolsState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
