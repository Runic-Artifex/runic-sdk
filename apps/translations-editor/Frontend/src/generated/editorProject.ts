...
export interface EditorProjectState {
  ...
  readonly previewProjectResultJson: string;
  readonly createProjectResultJson: string;
  readonly openWorkspaceResultJson: string;
}

export interface EditorProjectView {
  readonly snapshot: EditorProjectState;
  subscribe(listener: (state: EditorProjectState) => void): () => void;
  dispose(): void;
  previewProject(argument: string): Promise<EditorProjectState>;
  createProject(argument: string): Promise<EditorProjectState>;
  openWorkspace(argument: string): Promise<EditorProjectState>;
}

export interface EditorProjectPageReference {
  readonly kind: "editorProject";
  connect(): Promise<EditorProjectView>;
}
const pageEditorProjectReferences = new Map<string, WeakRef<EditorProjectPageReference>>();
const pageEditorProjectFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorProjectPageReference> }>(entry => {
  if (pageEditorProjectReferences.get(entry.id) === entry.reference) pageEditorProjectReferences.delete(entry.id);
});
export function pageEditorProject(id: string): EditorProjectPageReference {
  let reference = pageEditorProjectReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorProject", connect: () => connectEditorProjectAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorProjectReferences.set(id, weak);
  pageEditorProjectFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = EditorProjectState;
function hydrate(wire: WireState): EditorProjectState { return wire; }
...
const bridgeContract = "Runic.Translations.Editor.EditorProjectViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorProject(): Promise<EditorProjectView> { return connectEditorProjectAt("editorProject"); }
async function connectEditorProjectAt(route: string, needsMount = false): Promise<EditorProjectView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorProjectState {
    ...
    return state as EditorProjectState;
  }
  async function invoke(name: string, ...args: unknown[]): Promise<EditorProjectState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorProjectState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async previewProject(argument) {
      return invoke(`${route}PreviewProject`, JSON.stringify(argument));
    },
    async createProject(argument) {
      return invoke(`${route}CreateProject`, JSON.stringify(argument));
    },
    async openWorkspace(argument) {
      return invoke(`${route}OpenWorkspace`, JSON.stringify(argument));
    },
  };
}
