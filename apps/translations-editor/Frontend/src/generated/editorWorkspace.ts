...
export interface EditorWorkspaceState {
  ...
  readonly loadResultJson: string;
  readonly checkExternalChangesResultJson: string;
  readonly pickWorkspaceResultJson: string;
  readonly previewMutationResultJson: string;
  readonly applyMutationResultJson: string;
  readonly recoverTransactionResultJson: string;
  readonly undoResultJson: string;
  readonly redoResultJson: string;
}

export interface EditorWorkspaceView {
  readonly snapshot: EditorWorkspaceState;
  subscribe(listener: (state: EditorWorkspaceState) => void): () => void;
  dispose(): void;
  load(argument: string): Promise<EditorWorkspaceState>;
  checkExternalChanges(argument: string): Promise<EditorWorkspaceState>;
  pickWorkspace(argument: string): Promise<EditorWorkspaceState>;
  previewMutation(argument: string): Promise<EditorWorkspaceState>;
  applyMutation(argument: string): Promise<EditorWorkspaceState>;
  recoverTransaction(argument: string): Promise<EditorWorkspaceState>;
  undo(argument: string): Promise<EditorWorkspaceState>;
  redo(argument: string): Promise<EditorWorkspaceState>;
}

export interface EditorWorkspacePageReference {
  readonly kind: "editorWorkspace";
  connect(): Promise<EditorWorkspaceView>;
}
const pageEditorWorkspaceReferences = new Map<string, WeakRef<EditorWorkspacePageReference>>();
const pageEditorWorkspaceFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorWorkspacePageReference> }>(entry => {
  if (pageEditorWorkspaceReferences.get(entry.id) === entry.reference) pageEditorWorkspaceReferences.delete(entry.id);
});
export function pageEditorWorkspace(id: string): EditorWorkspacePageReference {
  let reference = pageEditorWorkspaceReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorWorkspace", connect: () => connectEditorWorkspaceAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorWorkspaceReferences.set(id, weak);
  pageEditorWorkspaceFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
const bridgeContract = "Runic.Translations.Editor.EditorWorkspaceViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorWorkspace(): Promise<EditorWorkspaceView> { return connectEditorWorkspaceAt("editorWorkspace"); }
async function connectEditorWorkspaceAt(route: string, needsMount = false): Promise<EditorWorkspaceView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorWorkspaceState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<EditorWorkspaceState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorWorkspaceState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
