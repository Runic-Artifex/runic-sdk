...
export interface EditorDocumentState {
  ...
  readonly validationResultJson: string;
  readonly saveResultJson: string;
}

export interface EditorDocumentView {
  readonly snapshot: EditorDocumentState;
  subscribe(listener: (state: EditorDocumentState) => void): () => void;
  dispose(): void;
  validate(argument: string): Promise<EditorDocumentState>;
  save(argument: string): Promise<EditorDocumentState>;
}

export interface EditorDocumentPageReference {
  readonly kind: "editorDocument";
  connect(): Promise<EditorDocumentView>;
}
const pageEditorDocumentReferences = new Map<string, WeakRef<EditorDocumentPageReference>>();
const pageEditorDocumentFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorDocumentPageReference> }>(entry => {
  if (pageEditorDocumentReferences.get(entry.id) === entry.reference) pageEditorDocumentReferences.delete(entry.id);
});
export function pageEditorDocument(id: string): EditorDocumentPageReference {
  let reference = pageEditorDocumentReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorDocument", connect: () => connectEditorDocumentAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorDocumentReferences.set(id, weak);
  pageEditorDocumentFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = EditorDocumentState;
function hydrate(wire: WireState): EditorDocumentState { return wire; }
...
const bridgeContract = "Runic.Translations.Editor.EditorDocumentViewModel:6BAA34FA704CDD4C8827DA54B90E3410B1CD9C90FFD3FF6F24D95AD6F1CF29C6";

export function connectEditorDocument(): Promise<EditorDocumentView> { return connectEditorDocumentAt("editorDocument"); }
async function connectEditorDocumentAt(route: string, needsMount = false): Promise<EditorDocumentView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorDocumentState {
    ...
    return state as EditorDocumentState;
  }
  async function invoke(name: string, ...args: unknown[]): Promise<EditorDocumentState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorDocumentState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async validate(argument) {
      return invoke(`${route}Validate`, JSON.stringify(argument));
    },
    async save(argument) {
      return invoke(`${route}Save`, JSON.stringify(argument));
    },
  };
}
