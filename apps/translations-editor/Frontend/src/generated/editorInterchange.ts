// <auto-generated />
export interface EditorInterchangeState {
  readonly revision: number;
  readonly exportXliffResultJson: string;
  readonly previewXliffImportResultJson: string;
  readonly applyXliffImportResultJson: string;
  readonly exportReviewJsonResultJson: string;
  readonly previewReviewJsonImportResultJson: string;
  readonly applyReviewJsonImportResultJson: string;
}

export interface EditorInterchangeView {
  readonly snapshot: EditorInterchangeState;
  subscribe(listener: (state: EditorInterchangeState) => void): () => void;
  dispose(): void;
  exportXliff(argument: string): Promise<EditorInterchangeState>;
  previewXliffImport(argument: string): Promise<EditorInterchangeState>;
  applyXliffImport(argument: string): Promise<EditorInterchangeState>;
  exportReviewJson(argument: string): Promise<EditorInterchangeState>;
  previewReviewJsonImport(argument: string): Promise<EditorInterchangeState>;
  applyReviewJsonImport(argument: string): Promise<EditorInterchangeState>;
}

export interface EditorInterchangePageReference {
  readonly kind: "editorInterchange";
  connect(): Promise<EditorInterchangeView>;
}
const pageEditorInterchangeReferences = new Map<string, WeakRef<EditorInterchangePageReference>>();
const pageEditorInterchangeFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorInterchangePageReference> }>(entry => {
  if (pageEditorInterchangeReferences.get(entry.id) === entry.reference) pageEditorInterchangeReferences.delete(entry.id);
});
export function pageEditorInterchange(id: string): EditorInterchangePageReference {
  let reference = pageEditorInterchangeReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorInterchange", connect: () => connectEditorInterchangeAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorInterchangeReferences.set(id, weak);
  pageEditorInterchangeFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = EditorInterchangeState;
function hydrate(wire: WireState): EditorInterchangeState { return wire; }
...
const bridgeContract = "Runic.Translations.Editor.EditorInterchangeViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorInterchange(): Promise<EditorInterchangeView> { return connectEditorInterchangeAt("editorInterchange"); }
async function connectEditorInterchangeAt(route: string, needsMount = false): Promise<EditorInterchangeView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorInterchangeState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<EditorInterchangeState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorInterchangeState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async exportXliff(argument) {
      return invoke(`${route}ExportXliff`, JSON.stringify(argument));
    },
    async previewXliffImport(argument) {
      return invoke(`${route}PreviewXliffImport`, JSON.stringify(argument));
    },
    async applyXliffImport(argument) {
      return invoke(`${route}ApplyXliffImport`, JSON.stringify(argument));
    },
    async exportReviewJson(argument) {
      return invoke(`${route}ExportReviewJson`, JSON.stringify(argument));
    },
    async previewReviewJsonImport(argument) {
      return invoke(`${route}PreviewReviewJsonImport`, JSON.stringify(argument));
    },
    async applyReviewJsonImport(argument) {
      return invoke(`${route}ApplyReviewJsonImport`, JSON.stringify(argument));
    },
  };
}
