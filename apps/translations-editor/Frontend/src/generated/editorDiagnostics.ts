...
export interface EditorDiagnosticsState {
  ...
  readonly aboutResultJson: string;
  readonly createDiagnosticBundleResultJson: string;
  readonly revealDiagnosticBundleResultJson: string;
  readonly deleteDiagnosticBundleResultJson: string;
}

export interface EditorDiagnosticsView {
  readonly snapshot: EditorDiagnosticsState;
  subscribe(listener: (state: EditorDiagnosticsState) => void): () => void;
  dispose(): void;
  about(argument: string): Promise<EditorDiagnosticsState>;
  createDiagnosticBundle(argument: string): Promise<EditorDiagnosticsState>;
  revealDiagnosticBundle(argument: string): Promise<EditorDiagnosticsState>;
  deleteDiagnosticBundle(argument: string): Promise<EditorDiagnosticsState>;
}

export interface EditorDiagnosticsPageReference {
  readonly kind: "editorDiagnostics";
  connect(): Promise<EditorDiagnosticsView>;
}
const pageEditorDiagnosticsReferences = new Map<string, WeakRef<EditorDiagnosticsPageReference>>();
const pageEditorDiagnosticsFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorDiagnosticsPageReference> }>(entry => {
  if (pageEditorDiagnosticsReferences.get(entry.id) === entry.reference) pageEditorDiagnosticsReferences.delete(entry.id);
});
export function pageEditorDiagnostics(id: string): EditorDiagnosticsPageReference {
  let reference = pageEditorDiagnosticsReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorDiagnostics", connect: () => connectEditorDiagnosticsAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorDiagnosticsReferences.set(id, weak);
  pageEditorDiagnosticsFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = EditorDiagnosticsState;
function hydrate(wire: WireState): EditorDiagnosticsState { return wire; }
...
const bridgeContract = "Runic.Translations.Editor.EditorDiagnosticsViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorDiagnostics(): Promise<EditorDiagnosticsView> { return connectEditorDiagnosticsAt("editorDiagnostics"); }
async function connectEditorDiagnosticsAt(route: string, needsMount = false): Promise<EditorDiagnosticsView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorDiagnosticsState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<EditorDiagnosticsState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorDiagnosticsState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async about(argument) {
      return invoke(`${route}About`, JSON.stringify(argument));
    },
    async createDiagnosticBundle(argument) {
      return invoke(`${route}CreateDiagnosticBundle`, JSON.stringify(argument));
    },
    async revealDiagnosticBundle(argument) {
      return invoke(`${route}RevealDiagnosticBundle`, JSON.stringify(argument));
    },
    async deleteDiagnosticBundle(argument) {
      return invoke(`${route}DeleteDiagnosticBundle`, JSON.stringify(argument));
    },
  };
}
