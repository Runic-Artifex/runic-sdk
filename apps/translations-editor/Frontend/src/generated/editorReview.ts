...
export interface EditorReviewState {
  ...
  readonly saveReviewResultJson: string;
}

export interface EditorReviewView {
  readonly snapshot: EditorReviewState;
  subscribe(listener: (state: EditorReviewState) => void): () => void;
  dispose(): void;
  saveReview(argument: string): Promise<EditorReviewState>;
}

export interface EditorReviewPageReference {
  readonly kind: "editorReview";
  connect(): Promise<EditorReviewView>;
}
const pageEditorReviewReferences = new Map<string, WeakRef<EditorReviewPageReference>>();
const pageEditorReviewFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorReviewPageReference> }>(entry => {
  if (pageEditorReviewReferences.get(entry.id) === entry.reference) pageEditorReviewReferences.delete(entry.id);
});
export function pageEditorReview(id: string): EditorReviewPageReference {
  let reference = pageEditorReviewReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorReview", connect: () => connectEditorReviewAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorReviewReferences.set(id, weak);
  pageEditorReviewFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = EditorReviewState;
function hydrate(wire: WireState): EditorReviewState { return wire; }
...
const bridgeContract = "Runic.Translations.Editor.EditorReviewViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorReview(): Promise<EditorReviewView> { return connectEditorReviewAt("editorReview"); }
async function connectEditorReviewAt(route: string, needsMount = false): Promise<EditorReviewView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorReviewState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<EditorReviewState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorReviewState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async saveReview(argument) {
      return invoke(`${route}SaveReview`, JSON.stringify(argument));
    },
  };
}
