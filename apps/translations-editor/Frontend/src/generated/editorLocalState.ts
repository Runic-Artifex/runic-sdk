// <auto-generated />
export interface EditorLocalStateState {
  readonly revision: number;
  readonly loadLocalStateResultJson: string;
  readonly saveLocalStateResultJson: string;
  readonly clearLocalStateResultJson: string;
}

export interface EditorLocalStateView {
  readonly snapshot: EditorLocalStateState;
  subscribe(listener: (state: EditorLocalStateState) => void): () => void;
  dispose(): void;
  loadLocalState(argument: string): Promise<EditorLocalStateState>;
  saveLocalState(argument: string): Promise<EditorLocalStateState>;
  clearLocalState(argument: string): Promise<EditorLocalStateState>;
}

export interface EditorLocalStatePageReference {
  readonly kind: "editorLocalState";
  connect(): Promise<EditorLocalStateView>;
}
const pageEditorLocalStateReferences = new Map<string, WeakRef<EditorLocalStatePageReference>>();
const pageEditorLocalStateFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<EditorLocalStatePageReference> }>(entry => {
  if (pageEditorLocalStateReferences.get(entry.id) === entry.reference) pageEditorLocalStateReferences.delete(entry.id);
});
export function pageEditorLocalState(id: string): EditorLocalStatePageReference {
  let reference = pageEditorLocalStateReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "editorLocalState", connect: () => connectEditorLocalStateAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageEditorLocalStateReferences.set(id, weak);
  pageEditorLocalStateFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = EditorLocalStateState;
function hydrate(wire: WireState): EditorLocalStateState { return wire; }
...
const bridgeContract = "Runic.Translations.Editor.EditorLocalStateViewModel:536E8D73F400A98F1C36B5D41B28EAC58437FF84641649C42910D0A1F6F9DD80";

export function connectEditorLocalState(): Promise<EditorLocalStateView> { return connectEditorLocalStateAt("editorLocalState"); }
async function connectEditorLocalStateAt(route: string, needsMount = false): Promise<EditorLocalStateView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): EditorLocalStateState {
    ...
    return state as EditorLocalStateState;
  }
  async function invoke(name: string, ...args: unknown[]): Promise<EditorLocalStateState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as EditorLocalStateState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async loadLocalState(argument) {
      return invoke(`${route}LoadLocalState`, JSON.stringify(argument));
    },
    async saveLocalState(argument) {
      return invoke(`${route}SaveLocalState`, JSON.stringify(argument));
    },
    async clearLocalState(argument) {
      return invoke(`${route}ClearLocalState`, JSON.stringify(argument));
    },
  };
}
