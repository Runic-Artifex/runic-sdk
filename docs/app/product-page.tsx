  ...
  return (
    <main>
      <section className="doc-hero shell">
        ...
        <div className="actions"><a className="button primary" href={product.source}>View source</a>{product.slug === "runic-toolkit" && <Link className="button secondary" href="/application-bridge">Application Bridge guide</Link>}<Link className="button secondary" href="/packages">Package catalog</Link></div>
      </section>
      ...
      <div className="doc-layout shell">
        ...
        <article className="doc-content">
          ...
          <section id="install"><p className="eyebrow">Preview</p><h2>Install the verified candidate</h2><div className="notice"><strong>Publication pending</strong><p>These exact versions pass the private package and application gates. The commands become public when repository visibility and registry trust are enabled together.</p></div>{product.install?.map((command) => <pre key={command}><code>{command}</code></pre>)}</section>
