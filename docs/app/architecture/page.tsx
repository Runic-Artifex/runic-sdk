...
export default function ArchitecturePage() {
  return <main><section className="page-hero shell"><p className="eyebrow">Architecture</p><h1>Ownership and dependency direction are separate decisions.</h1><p className="lede">A product owns the behavior it contributes. An integration depends on both cores. Neither core depends back on the integration.</p></section><section className="content-grid shell">
    ...
    <article className="info-card"><p className="eyebrow">Product autonomy</p><h2>Independent histories</h2><p>Markup, Flow, Assets, Text Resources, Command Line, Toolkit, and CsWebUi each version and release from their own repository. A change in one product does not pollute every other product history.</p></article>
    ...
    <article className="info-card"><p className="eyebrow">Framework neutrality</p><h2>Cores stay portable</h2><p>Runic Flow has no UI dependency. Runic Assets has no host dependency. Runic Markup has no required UI framework. Runic Text Resources begins with .NET but defines language-neutral contracts.</p></article>
    <article className="info-card"><p className="eyebrow">Integration ownership</p><h2>Behavior lives with its author</h2><p><code>RunicAssets.RunicToolkit</code>, <code>RunicFlow.RunicToolkit</code>, and <code>RunicMarkup.RunicToolkit.*</code> live and release with Assets, Flow, and Markup respectively.</p></article>
  </section></main>;
}
