...
export default function ApplicationBridgePage() {
  return <main>
    <section className="page-hero shell">
      ...
      <div className="actions"><a className="button primary" href="https://github.com/Runic-Artifex/runic-toolkit-examples/tree/main/samples/03-SetupApplication">Study the Setup vertical</a><a className="button secondary" href="https://github.com/Runic-Artifex/runic-toolkit/blob/main/docs/guides/application-bridge.md">Repository guide</a></div>
    </section>
    <section className="content-grid shell">
      ...
      <article className="info-card full"><p className="eyebrow">Install</p><h2>Consume the verified private candidate</h2><pre><code>dotnet add package RunicToolkit.Hosting.CsWebUi.ApplicationBridge --version 0.1.0-preview.13.1</code></pre><pre><code>dotnet add package RunicToolkit.ApplicationBridge.Generators --version 0.1.0-preview.13.1</code></pre><pre><code>npm install @runic-artifex/application-bridge@0.1.0-preview.13.1 effect@3.22.1</code></pre><p>The package commands become public at launch. Until then, authenticated GitHub Packages access is required.</p></article>
      <article className="info-card full"><p className="eyebrow">Reference proof</p><h2>The neutral Setup application closes the loop</h2><p>The package-only reference application covers initialization, backend-authoritative navigation, opaque destination selection, installation receipts, progress, completion, failure, cancellation, recovery, production frontend output, NativeAOT, and a real Windows native-browser roundtrip.</p></article>
    </section>
  </main>;
}
