...
| Windows | Use the [Windows UI Automation smoke](../../../tests/native/Runic.Desktop.WebViewSmoke/windows-ui-automation.md) through the existing interactive VM login. It verifies WebView2 accessibility names, editable focus, `ValuePattern`, `InvokePattern`, live output and deterministic process exit for JIT and executable-only NativeAOT publishes. Run native power-request checks independently. Session-0 SSH alone cannot cover interactive display behavior. Keyboard navigation/IME, DPI and pointer targeting, Narrator speech, native picker interaction, and visual rendering remain unautomated Windows work. |
