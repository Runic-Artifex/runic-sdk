...
| Windows | Use the [Windows UI Automation smoke](../../../tests/native/Runic.Desktop.WebViewSmoke/windows-ui-automation.md) through the existing interactive VM login. It verifies WebView2 accessibility names, editable focus, `ValuePattern`, `InvokePattern`, actual typed text and Tab/Shift+Tab/Enter behavior through an accessible focus output, a UIA-point mouse click at the VM's observed 96 DPI, native open-file selection, live output and deterministic process exit for JIT and executable-only NativeAOT publishes. Run native power-request checks independently. Session-0 SSH alone cannot cover interactive display behavior. WebView2 button `HasKeyboardFocus` reporting, IME composition, Narrator speech, physical input, display-scale changes/multi-DPI pointer behavior, picker cancel/save flows and visual rendering remain Windows work. |
