...
[RMF2 validation](rmf2-validation.md). WPF cross-compiles; Windows control
execution remains pending. Cross-language boundaries and the two IDE packages
remain in progress. The source-only input/slot commands return versioned
workspace edits and do not imply application-call-site changes.
