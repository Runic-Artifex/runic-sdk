...
export interface CounterView {
  ...
  startIncrement(): Promise<CounterIncrementOperation>;
...
export interface CounterIncrementOperation {
...
type WireState = CounterState;
function hydrate(wire: WireState): CounterState { return wire; }
...
const bridgeContract = "FirstWindowDesktop.CounterViewModel:11AD50D46C441D263DA57F740EF78F0C7650A5723690B84C7B096C80D30CF20B";
...
async function connectCounterAt(route: string, needsMount = false): Promise<CounterView> {
  ...
  function incrementOperation(requestId: string, terminal?: BridgeOperationStatus): CounterIncrementOperation {
  ...
  async function startIncrementWithRequestId(requestId: string): Promise<CounterIncrementOperation> {
    ...
    if (prior?.admission) return prior.admission as Promise<CounterIncrementOperation>;
    if (lease.disposed || !isLive() || !bridge.isConnected()) throw new BridgeError("disconnected", "The Bridge is disconnected.");
    ...
    const admission = (async () => {
      let reply: string;
      try { reply = await bridge.call(`${route}StartIncrement`, requestId); }
      ...
      if (result.kind === "accepted" || result.kind === "duplicate") {
        let terminal: BridgeOperationStatus | undefined; try { terminal = operationFromInline(result.terminal, requestId); } catch (error) { tracked.state = "uncertain"; throw error; } const handle = incrementOperation(requestId, terminal); tracked.state = "accepted"; tracked.handle = handle; return handle;
      }
  ...
  async function recoverIncrementWithRequestId(requestId: string): Promise<CounterIncrementOperation> {
    if (requestId.length === 0) throw new RangeError("Operation requestId is required.");
    if (lease.disposed || !isLive() || !bridge.isConnected()) throw new BridgeError("disconnected", "The Bridge is disconnected.");
    ...
    const handle = incrementOperation(requestId, terminal);
    const key = operationKey(requestId);
  ...
  return {
    ...
    startIncrement() {
      return startIncrementWithRequestId(globalThis.crypto.randomUUID());
    },
    startIncrementWithRequestId(requestId) {
      return startIncrementWithRequestId(requestId);
    },
    recoverIncrementWithRequestId(requestId) {
      return recoverIncrementWithRequestId(requestId);
    },
  };
}
