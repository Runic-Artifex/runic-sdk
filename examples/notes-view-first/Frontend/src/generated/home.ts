...
export interface HomePageReference {
  readonly kind: "home";
  connect(): Promise<HomeView>;
}
const pageHomeReferences = new Map<string, WeakRef<HomePageReference>>();
const pageHomeFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<HomePageReference> }>(entry => {
  if (pageHomeReferences.get(entry.id) === entry.reference) pageHomeReferences.delete(entry.id);
});
export function pageHome(id: string): HomePageReference {
  let reference = pageHomeReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "home", connect: () => connectHomeAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageHomeReferences.set(id, weak);
  pageHomeFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = HomeState;
function hydrate(wire: WireState): HomeState { return wire; }
...
const bridgeContract = "NotesWindowViews.HomeViewModel:935E964C4D6167F9EFACE80F3E1A77D03CD7B5C219DF2F98CAA393C9B572C50B";
...
async function connectHomeAt(route: string, needsMount = false): Promise<HomeView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): HomeState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<HomeState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as HomeState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
