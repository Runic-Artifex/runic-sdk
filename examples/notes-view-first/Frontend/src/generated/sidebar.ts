...
export interface SidebarView {
  readonly snapshot: SidebarState;
...
export interface SidebarPageReference {
  ...
  connect(): Promise<SidebarView>;
}
const pageSidebarReferences = new Map<string, WeakRef<SidebarPageReference>>();
const pageSidebarFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<SidebarPageReference> }>(entry => {
  if (pageSidebarReferences.get(entry.id) === entry.reference) pageSidebarReferences.delete(entry.id);
});
export function pageSidebar(id: string): SidebarPageReference {
  let reference = pageSidebarReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "sidebar", connect: () => connectSidebarAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageSidebarReferences.set(id, weak);
  pageSidebarFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = SidebarState;
function hydrate(wire: WireState): SidebarState { return wire; }
...
const bridgeContract = "NotesWindowViews.SidebarViewModel:935E964C4D6167F9EFACE80F3E1A77D03CD7B5C219DF2F98CAA393C9B572C50B";

export function connectSidebar(): Promise<SidebarView> { return connectSidebarAt("sidebar"); }
async function connectSidebarAt(route: string, needsMount = false): Promise<SidebarView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): SidebarState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<SidebarState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as SidebarState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
