...
export interface ConfirmNavigationView {
  readonly snapshot: ConfirmNavigationState;
  subscribe(listener: (state: ConfirmNavigationState) => void): () => void;
  dispose(): void;
...
export interface ConfirmNavigationPageReference {
  ...
  connect(): Promise<ConfirmNavigationView>;
}
const pageConfirmNavigationReferences = new Map<string, WeakRef<ConfirmNavigationPageReference>>();
const pageConfirmNavigationFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<ConfirmNavigationPageReference> }>(entry => {
  if (pageConfirmNavigationReferences.get(entry.id) === entry.reference) pageConfirmNavigationReferences.delete(entry.id);
});
export function pageConfirmNavigation(id: string): ConfirmNavigationPageReference {
  let reference = pageConfirmNavigationReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "confirmNavigation", connect: () => connectConfirmNavigationAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pageConfirmNavigationReferences.set(id, weak);
  pageConfirmNavigationFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = ConfirmNavigationState;
function hydrate(wire: WireState): ConfirmNavigationState { return wire; }
...
const bridgeContract = "NotesWindowViews.ConfirmNavigationViewModel:935E964C4D6167F9EFACE80F3E1A77D03CD7B5C219DF2F98CAA393C9B572C50B";

export function connectConfirmNavigation(): Promise<ConfirmNavigationView> { return connectConfirmNavigationAt("confirmNavigation"); }
async function connectConfirmNavigationAt(route: string, needsMount = false): Promise<ConfirmNavigationView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): ConfirmNavigationState {
    ...
    return state as ConfirmNavigationState;
  }
  async function invoke(name: string, ...args: unknown[]): Promise<ConfirmNavigationState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as ConfirmNavigationState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
    ...
    dispose,
    async confirm() {
      return invoke(`${route}Confirm`);
    },
    async cancel() {
      return invoke(`${route}Cancel`);
    },
  };
}
