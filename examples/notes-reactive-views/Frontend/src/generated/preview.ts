...
export interface PreviewView {
  readonly snapshot: PreviewState;
...
export interface PreviewPageReference {
  ...
  connect(): Promise<PreviewView>;
}
const pagePreviewReferences = new Map<string, WeakRef<PreviewPageReference>>();
const pagePreviewFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<PreviewPageReference> }>(entry => {
  if (pagePreviewReferences.get(entry.id) === entry.reference) pagePreviewReferences.delete(entry.id);
});
export function pagePreview(id: string): PreviewPageReference {
  let reference = pagePreviewReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "preview", connect: () => connectPreviewAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pagePreviewReferences.set(id, weak);
  pagePreviewFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = PreviewState;
function hydrate(wire: WireState): PreviewState { return wire; }
...
const bridgeContract = "NotesReactiveViews.PreviewViewModel:C57A5A1D229FD660284E597E0ACA7DDB62F5F4ADF32FCE1157BAC5080931D55A";

export function connectPreview(): Promise<PreviewView> { return connectPreviewAt("preview"); }
async function connectPreviewAt(route: string, needsMount = false): Promise<PreviewView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): PreviewState {
  ...
  async function invoke(name: string, ...args: unknown[]): Promise<PreviewState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as PreviewState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
