// <auto-generated />
export interface PinnedTaskState {
  readonly revision: number;
  readonly priority: string;
}

export interface PinnedTaskView {
  readonly snapshot: PinnedTaskState;
  subscribe(listener: (state: PinnedTaskState) => void): () => void;
  dispose(): void;
}

export interface PinnedTaskPageReference {
  readonly kind: "pinnedTask";
  connect(): Promise<PinnedTaskView>;
}
const pagePinnedTaskReferences = new Map<string, WeakRef<PinnedTaskPageReference>>();
const pagePinnedTaskFinalizer = new FinalizationRegistry<{ id: string; reference: WeakRef<PinnedTaskPageReference> }>(entry => {
  if (pagePinnedTaskReferences.get(entry.id) === entry.reference) pagePinnedTaskReferences.delete(entry.id);
});
export function pagePinnedTask(id: string): PinnedTaskPageReference {
  let reference = pagePinnedTaskReferences.get(id)?.deref();
  if (reference) return reference;
  reference = { kind: "pinnedTask", connect: () => connectPinnedTaskAt(`content${id}`, true) };
  const weak = new WeakRef(reference);
  pagePinnedTaskReferences.set(id, weak);
  pagePinnedTaskFinalizer.register(reference, { id, reference: weak });
  return reference;
}
...
type WireState = PinnedTaskState;
function hydrate(wire: WireState): PinnedTaskState { return wire; }
...
const bridgeContract = "NotesReactiveViews.PinnedTaskViewModel:C57A5A1D229FD660284E597E0ACA7DDB62F5F4ADF32FCE1157BAC5080931D55A";

export function connectPinnedTask(): Promise<PinnedTaskView> { return connectPinnedTaskAt("pinnedTask"); }
async function connectPinnedTaskAt(route: string, needsMount = false): Promise<PinnedTaskView> {
  ...
  function unpack(json: string, target: SharedEntry = shared): PinnedTaskState {
    ...
    return state as PinnedTaskState;
  }
  async function invoke(name: string, ...args: unknown[]): Promise<PinnedTaskState> {
  ...
  return {
    get snapshot() {
      ...
      return lease.current as PinnedTaskState;
    },
    subscribe(listener) {
      if (lease.disposed || !isLive() || lease.current === undefined) throw new BridgeError("disconnected", "ViewModel is not connected.");
