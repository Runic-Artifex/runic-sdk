    ...
    where THandler : notnull, ICommandHandler<TOptions, TResult>
{
    ...
    internal override void ValidateRegistration(string location, List<CommandCatalogIssue> issues)
    {
        ...
        if (Binder is null)
        {
            issues.Add(new CommandCatalogIssue("WUTCLI0010", location, "A closed options binder is required."));
        }

        if (HandlerFactory is null)
        {
            issues.Add(new CommandCatalogIssue("WUTCLI0011", location, "A closed handler factory is required."));
        }

        if (ResultCodec is null)
        {
            issues.Add(new CommandCatalogIssue("WUTCLI0012", location, "A closed result codec is required."));
        }
        else
        {
            try
...
internal abstract class CommandBuilderNode
{
    ...
    internal void Validate(string path, List<CommandCatalogIssue> issues)
    {
        ...
        {
            issues.Add(new CommandCatalogIssue("WUTCLI0003", path, "The description key cannot be empty."));
        }
    ...
    private void ValidateOptions(string path, List<CommandCatalogIssue> issues)
    {
        ...
        foreach (OptionDefinition option in _options)
        {
            ...
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0005", path, $"Option ID '{option.Id}' is invalid."));
            }

            if (!ids.Add(option.Id))
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0006", path, $"Option ID '{option.Id}' is registered more than once."));
            }
            ...
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0009", path, $"Option '{option.Name}' has an invalid repeat policy."));
            }

            if (option.DescriptionKey is not null && string.IsNullOrWhiteSpace(option.DescriptionKey))
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0003", path, $"Option '{option.Name}' has an empty description key."));
            }
        }
        ...
        void ValidateOptionSpelling(string spelling)
        {
            ...
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0007", path, $"Option spelling '{spelling}' is invalid or reserved."));
            }

            if (!spellings.Add(spelling))
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0008", path, $"Option spelling '{spelling}' is registered more than once."));
            }
        }
    }
    ...
    private void ValidateArguments(string path, List<CommandCatalogIssue> issues)
    {
        ...
        for (int index = 0; index < _arguments.Count; index++)
        {
            ...
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0013", path, $"Argument '{argument.Name}' has an invalid name or ID."));
            }

            if (!ids.Add(argument.Id))
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0014", path, $"Argument ID '{argument.Id}' is registered more than once."));
            }

            if (argument.DescriptionKey is not null && string.IsNullOrWhiteSpace(argument.DescriptionKey))
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0003", path, $"Argument '{argument.Name}' has an empty description key."));
            }

            if (optionalSeen && argument.Arity.Minimum != 0)
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0015", path, "A required argument cannot follow an optional argument."));
            }

            optionalSeen |= argument.Arity.Minimum == 0;
            if (argument.Arity.Maximum is null && index != _arguments.Count - 1)
            {
                issues.Add(new CommandCatalogIssue("WUTCLI0016", path, "Only the final argument may have unbounded arity."));
            }
        }
    }
    ...
    private static void ValidateCommandName(string name, string path, List<CommandCatalogIssue> issues)
    {
        ...
        {
            issues.Add(new CommandCatalogIssue("WUTCLI0002", path, $"Command name or alias '{name}' is invalid or reserved."));
        }
    }
