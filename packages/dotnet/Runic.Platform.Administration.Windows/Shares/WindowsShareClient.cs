...
public sealed partial class WindowsShareClient
{
    private readonly string? _server;
    /// <summary>Creates a share client; null targets the local computer.</summary>
    public WindowsShareClient(string? server = null)
    {
        NativeError.Windows();
    ...
    public unsafe ShareSnapshot? Find(string name)
    {
        ...
        try
        {
            ...
            return new(Text(value->Name), Text(value->Path), Text(value->Remark), value->Type,
                value->MaximumUses, value->CurrentUses, ReadDescriptor(value->Descriptor));
        }
        finally { if (buffer != 0) _ = NetApiBufferFree(buffer); }
    }
    ...
    public unsafe void Create(ShareSpecification specification)
    {
        ...
        var descriptor = specification.SecurityDescriptor is { } supplied ? ValidateDescriptor(supplied) : null;
    ...
    public unsafe void Update(string name, ShareUpdate update)
    {
        ...
        var descriptor = update.SecurityDescriptor is { } supplied ? ValidateDescriptor(supplied) : null;
    ...
    private static unsafe byte[] ValidateDescriptor(ImmutableArray<byte> descriptor)
    {
        ...
        fixed (byte* pointer = bytes)
        {
            ...
            if (IsValidSecurityDescriptor(pointer) == 0 ||
                GetSecurityDescriptorControl(pointer, out var control, out _) == 0 || (control & 0x8000) == 0 ||
                GetSecurityDescriptorLength(pointer) != bytes.Length)
                throw new ArgumentException("Supply a complete self-relative security descriptor.", nameof(descriptor));
        }
        return bytes;
    }

    private static unsafe ImmutableArray<byte> ReadDescriptor(nint descriptor)
    {
        if (descriptor == 0) throw NativeError.Win32("Read share security descriptor", 13);
        NativeError.CheckWin32(GetSecurityDescriptorControl((void*)descriptor, out var control, out _), "Read share security descriptor");
        if ((control & 0x8000) != 0)
        {
            var length = checked((int)GetSecurityDescriptorLength((void*)descriptor));
            return [.. new ReadOnlySpan<byte>((void*)descriptor, length)];
        }
        uint needed = 0;
        var status = MakeSelfRelativeSD((void*)descriptor, null, ref needed);
        ...
        fixed (byte* pointer = buffer)
            NativeError.CheckWin32(MakeSelfRelativeSD((void*)descriptor, pointer, ref needed), "Copy share security descriptor");
        return [.. buffer];
    }
    ...
    [LibraryImport("advapi32.dll", SetLastError = true)] private static unsafe partial int GetSecurityDescriptorControl(void* descriptor, out ushort control, out uint revision);
    [LibraryImport("advapi32.dll")] private static unsafe partial uint GetSecurityDescriptorLength(void* descriptor);
    [LibraryImport("advapi32.dll")] private static unsafe partial int IsValidSecurityDescriptor(void* descriptor);
    [LibraryImport("advapi32.dll", SetLastError = true)] private static unsafe partial int MakeSelfRelativeSD(void* absolute, void* relative, ref uint length);
}
