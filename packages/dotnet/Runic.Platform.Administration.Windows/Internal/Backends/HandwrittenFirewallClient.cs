...
internal sealed class HandwrittenFirewallClient : IWindowsFirewallClient
{
    /// <summary>Creates a local firewall client.</summary>
    public HandwrittenFirewallClient() => Automation.RequireX64();
    ...
    private static string FieldName(int slot) => slot switch
    {
        8 => "name", 10 => "description", 12 => "application", 14 => "service",
        16 => "protocol", 18 => "local ports", 20 => "remote ports", 22 => "local addresses",
        24 => "remote addresses", 26 => "ICMP types/codes", 28 => "direction",
        32 => "interface types", 36 => "grouping", 38 => "profiles", 42 => "action",
        _ => $"field {slot}"
    };
    ...
    private static unsafe void SetText(ComObject rule, int slot, string value)
    {
        ...
        NativeError.Check(((delegate* unmanaged[Stdcall]<nint, nint, int>)rule.Slot(slot))(rule.Pointer, text.Pointer), $"Set firewall {FieldName(slot)}");
    }

    private static unsafe void SetInt(ComObject rule, int slot, int value) =>
        NativeError.Check(((delegate* unmanaged[Stdcall]<nint, int, int>)rule.Slot(slot))(rule.Pointer, value), $"Set firewall {FieldName(slot)}");
    ...
    private static unsafe void Create(ComObject policy, ComObject rules, FirewallRuleSpecification specification)
    {
        ...
        var status = ((delegate* unmanaged[Stdcall]<nint, nint, int>)rules.Slot(8))(rules.Pointer, rule.Pointer);
        if (status < 0)
        {
