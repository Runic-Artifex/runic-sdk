...
- The current target accepts exactly one `runic.json` project and its discovered sibling `{locale}.toml` files (`sourceLayout: "locale-toml"`) or legacy `.mf2` messages.
