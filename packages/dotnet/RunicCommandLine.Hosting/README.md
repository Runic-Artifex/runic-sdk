...
`RunicCommandLine.Hosting` is the host-neutral handoff around the
CommandLine kernel. It references only `RunicCommandLine`; it does not
...
which snapshots arguments and the `RUNIC_COMMANDLINE_OUTPUT` value before any
