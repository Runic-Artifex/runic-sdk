    ...
    where THandler : notnull, ICommandHandler<TOptions, TResult>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<CommandDiagnostic> diagnostics = NoDiagnostics;
        ...
        try
        {
            try
            {
                ...
                else
                {
                    ...
                    diagnostics = MergeDiagnostics(binding.Diagnostics, outcome.Diagnostics);
                }
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
            {
                outcome = CommandOutcome.Failure<TResult>(
                    ...
                    new CommandFault("RCLI4000", "The command was cancelled."));
    ...
    private static CommandOutcome<TResult> HostFailure() => CommandOutcome.Failure<TResult>(
        ...
        new CommandFault("RCLI5000", "The command could not be completed."));

    private static IReadOnlyList<CommandDiagnostic> MergeDiagnostics(
        IReadOnlyList<CommandDiagnostic> bindingDiagnostics,
        IReadOnlyList<CommandDiagnostic> outcomeDiagnostics)
    {
        ...
        if (outcomeDiagnostics.Count == 0)
        {
            return bindingDiagnostics;
        }

        var merged = new CommandDiagnostic[bindingDiagnostics.Count + outcomeDiagnostics.Count];
        for (int index = 0; index < bindingDiagnostics.Count; index++)
        {
            merged[index] = bindingDiagnostics[index];
        }

        for (int index = 0; index < outcomeDiagnostics.Count; index++)
        {
            merged[bindingDiagnostics.Count + index] = outcomeDiagnostics[index];
        }

        return Array.AsReadOnly(merged);
    }
