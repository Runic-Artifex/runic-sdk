...
public sealed class ParseSettings
{
    ...
    /// <param name="transportOutputOptionName">
    /// The framework-owned long option used to select human or JSON output.
    /// </param>
        ...
        string transportOutputOptionName = "--output")
    {
        ...
        if (!IsLongOptionSpelling(transportOutputOptionName) || IsReservedTransportOption(transportOutputOptionName))
        {
            throw new ArgumentException(
                "The transport output option must be a non-reserved long option with a valid identifier.",
                nameof(transportOutputOptionName));
        }
    ...
    /// <summary>Gets the framework-owned option used to select output transport.</summary>
    public string TransportOutputOptionName { get; }

    private static bool IsLongOptionSpelling(string? value)
    {
        if (value is null || value.Length < 3 || !value.StartsWith("--", StringComparison.Ordinal) ||
            value[2] is < 'a' or > 'z')
        {
            return false;
        }
    ...
    private static bool IsReservedTransportOption(string value) =>
        value is "--help" or "--version";
}
...
public sealed class ParseOutcome
{
    ...
    /// <summary>
    /// Gets the frozen output classification selected while parsing. It is null
    /// only for callers that construct a syntax error without a classification.
    /// </summary>
    public CommandOutputClassification? OutputClassification { get; }
    ...
    /// <summary>
    /// Creates a syntax error result while preserving the parser-selected
    /// output classification for presentation.
    /// </summary>
    /// <param name="diagnostic">The safe parser diagnostic.</param>
    /// <param name="outputClassification">The valid frozen output selection reached while parsing.</param>
    /// <returns>A syntax error result.</returns>
    public static ParseOutcome FromError(
