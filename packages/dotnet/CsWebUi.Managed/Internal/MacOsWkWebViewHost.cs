...
internal sealed partial class MacOsWkWebViewHost : IWebUiEmbeddedHost, IWebUiMainThreadHost
{
    ...
    private static unsafe ValueTask InvokeOnMainAsync(Action action, CancellationToken cancellationToken)
    {
        if (IsMainThread)
        {
            action();
    ...
    private static unsafe nint CreateDelegateClass()
    {
        ...
        var existing = Api.GetClass("CsWebUiManagedWindowDelegate");
        if (existing != 0)
        {
            return existing;
        }
        var type = Api.AllocateClassPair(Api.GetClass("NSObject"), "CsWebUiManagedWindowDelegate");
        if (type == 0)
        {
            return 0;
        }
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    private static void RunMainQueueWorkItem(nint context)
    {
        var handle = GCHandle.FromIntPtr(context);
        var work = (MainQueueWorkItem)handle.Target!;
        handle.Free();
        try
        {
            work.Action();
