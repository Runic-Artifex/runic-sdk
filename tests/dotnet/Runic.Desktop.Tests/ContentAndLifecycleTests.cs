...
public sealed class ContentAndLifecycleTests
{
    ...
    {
        var root = Directory.CreateTempSubdirectory("runic-desktop-content-");
        try
        {
    ...
    {
        var root = Directory.CreateTempSubdirectory("runic-desktop-file-");
        try
        {
    ...
    {
        var root = Directory.CreateTempSubdirectory("runic-desktop-virtual-");
        try
        {
    ...
    public async Task RejectsTraversalAndSymlinksOutsideRoot()
    {
        ...
        var parent = Directory.CreateTempSubdirectory("runic-desktop-scope-");
        try
        {
    ...
    {
        var firstRoot = Directory.CreateTempSubdirectory("runic-desktop-root-one-");
        var secondRoot = Directory.CreateTempSubdirectory("runic-desktop-root-two-");
        try
        {
    ...
    {
        var firstRoot = Directory.CreateTempSubdirectory("runic-desktop-live-root-one-");
        var secondRoot = Directory.CreateTempSubdirectory("runic-desktop-live-root-two-");
        try
        {
            await File.WriteAllTextAsync(Path.Combine(firstRoot.FullName, "value.txt"), "one");
            await File.WriteAllTextAsync(Path.Combine(secondRoot.FullName, "value.txt"), "two");
