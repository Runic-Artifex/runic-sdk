...
if (args.Contains("--native-services", StringComparer.Ordinal) || args.Contains("--native-select", StringComparer.Ordinal))
{
    bool manualDesktopServices = args.Contains("--native-services", StringComparer.Ordinal);
    bool manualSelection = args.Contains("--native-select", StringComparer.Ordinal);
    if (OperatingSystem.IsMacOS())
    {
        int exitCode = 1;
        DesktopEventLoop.Run(async () =>
        {
            exitCode = await NativePlatformSmoke.RunAsync(manualDesktopServices, manualSelection);
        });
        return exitCode;
    }
    return await NativePlatformSmoke.RunAsync(manualDesktopServices, manualSelection);
}
