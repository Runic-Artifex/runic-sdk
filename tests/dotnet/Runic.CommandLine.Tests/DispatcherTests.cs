...
internal static class DispatcherTests
{
    ...
    [
        ...
        new("dispatcher/omits-failure-human-output-from-json", JsonOmitsFailureHumanOutput),
        new("dispatcher/failure-human-output-sink-failure-is-host-failure", FailureHumanOutputSinkFailure),
    ...
    private static async ValueTask BindingFault()
    {
        ...
        AssertEx.Equal(1, scopes.Scopes.Single().DisposeCount);
        AssertEx.Equal(0, sink.Diagnostics.Count);
    }

    private static async ValueTask MergesDiagnosticsOnce()
    {
        CommandDiagnostic bindingDiagnostic = new(
            "RCLI2001", "binding-warning", "The bound value is deprecated.",
            CommandDiagnosticPhase.Binding, CommandDiagnosticSeverity.Warning);
        CommandDiagnostic firstHandlerDiagnostic = new(
            "RCLI3001", "first-validation", "The first value is invalid.",
            CommandDiagnosticPhase.Execution, CommandDiagnosticSeverity.Error);
        CommandDiagnostic secondHandlerDiagnostic = new(
            "RCLI3002", "second-validation", "The second value is invalid.",
            CommandDiagnosticPhase.Execution, CommandDiagnosticSeverity.Error);
        var binder = new TestBinder((_, _) => ValueTask.FromResult(CommandOutcome.Success(
            new TestOptions(), [bindingDiagnostic])));
        var factory = new TestHandlerFactory(_ => new TestHandler((_, _, _) => ValueTask.FromResult(
            CommandOutcome.Failure<TestResult>(
                CommandExitCategory.Validation,
                new CommandFault("RCLI3000", "The command validation failed."),
                [firstHandlerDiagnostic, secondHandlerDiagnostic]))));
        var scopes = new TrackingScopeFactory();
        (CommandExecutor executor, CommandExecutionRequest request) = CreateExecution(
            binder, factory, scopes, new MemoryCommandConsole(), "merged-diagnostics");
        var sink = new CapturingSink();
        ...
        AssertEx.SequenceEqual(
            ["RCLI2001", "RCLI3001", "RCLI3002"],
            sink.Diagnostics.Select(static diagnostic => diagnostic.Code));
    }
        ...
        var factory = new TestHandlerFactory(_ => new TestHandler((_, _, _) =>
            ValueTask.FromResult(CommandOutcome.Failure<TestResult>(
                ...
                [new CommandDiagnostic(
                    "RCLI3001", "command-failed", "Command failed.",
                    CommandDiagnosticPhase.Execution, CommandDiagnosticSeverity.Error)],
                "dry-run report\n"))));
        (CommandExecutor executor, CommandExecutionRequest request) = CreateExecution(
    ...
    private static async ValueTask FailureHumanOutput()
    {
        var factory = new TestHandlerFactory(_ => new TestHandler((_, _, _) => ValueTask.FromResult(
            CommandOutcome.Failure<TestResult>(
                CommandExitCategory.Validation,
                new CommandFault("RCLI3000", "The dry run failed."),
                [new CommandDiagnostic(
                    "RCLI3001", "dry-run-invalid", "The dry run report is incomplete.",
                    CommandDiagnosticPhase.Execution, CommandDiagnosticSeverity.Error)],
                "dry-run report\n"))));
        ...
        var executor = new CommandExecutor(new TrackingScopeFactory());

        CommandExecutionResult result = await executor.ExecuteAsync(
            new CommandExecutionRequest(invocation, console, CultureInfo.InvariantCulture, "human-prelude"),
            new CommandOutputDispatcher());

        AssertEx.Equal(CommandExitCategory.Validation, result.ExitCategory);
        AssertEx.Equal(CommandExitCodes.Validation, result.ExitCode);
        AssertEx.Equal("dry-run report\n", console.StandardOutput);
        AssertEx.Equal(
            "RCLI3001: The dry run report is incomplete.\nRCLI3000: The dry run failed.\n",
            console.StandardError);
    }

    private static async ValueTask JsonOmitsFailureHumanOutput()
    {
        var factory = new TestHandlerFactory(_ => new TestHandler((_, _, _) => ValueTask.FromResult(
            CommandOutcome.Failure<TestResult>(
                CommandExitCategory.Validation,
                ...
                "dry-run report\n"))));
        CommandCatalog catalog = FixtureCatalog.Create(new TestBinder(), factory, new TestCodec());
        ParsedInvocation invocation = FixtureCatalog.ParseInvocation(catalog, "status", "--output=json");
        var console = new MemoryCommandConsole();
        var executor = new CommandExecutor(new TrackingScopeFactory());

        CommandExecutionResult result = await executor.ExecuteAsync(
            new CommandExecutionRequest(invocation, console, CultureInfo.InvariantCulture, "json-prelude"),
            new CommandOutputDispatcher());

        AssertEx.Equal(CommandExitCategory.Validation, result.ExitCategory);
        AssertEx.Equal(string.Empty, console.StandardError);
        AssertEx.True(!console.StandardOutput.Contains("dry-run report", StringComparison.Ordinal));
        AssertEx.True(console.StandardOutput.Contains("\"success\":false", StringComparison.Ordinal));
        AssertEx.True(console.StandardOutput.Contains("\"RCLI3001\"", StringComparison.Ordinal));
    }

    private static async ValueTask FailureHumanOutputSinkFailure()
    {
        var factory = new TestHandlerFactory(_ => new TestHandler((_, _, _) => ValueTask.FromResult(
            CommandOutcome.Failure<TestResult>(
                CommandExitCategory.Validation,
        ...
        CommandExecutionResult result = await executor.ExecuteAsync(
            new CommandExecutionRequest(invocation, new ThrowingOutputConsole(), CultureInfo.InvariantCulture, "human-prelude-sink"),
            new CommandOutputDispatcher());

        AssertEx.Equal(CommandExitCategory.HostFailure, result.ExitCategory);
        AssertEx.Equal(CommandExitCodes.HostFailure, result.ExitCode);
    }
    ...
    private sealed class ThrowingOutputConsole : ICommandConsole
    {
        ...
        public ValueTask WriteOutAsync(ReadOnlyMemory<char> value, CancellationToken cancellationToken) =>
            ValueTask.FromException(new InvalidOperationException("human output failed"));
