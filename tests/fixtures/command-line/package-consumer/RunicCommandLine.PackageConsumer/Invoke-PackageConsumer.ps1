...
$ownedProjects = @(
    ...
    (Join-Path $repositoryRoot 'src/RunicCommandLine/RunicCommandLine.csproj'),
    (Join-Path $repositoryRoot 'src/RunicCommandLine.Hosting/RunicCommandLine.Hosting.csproj'),
    (Join-Path $repositoryRoot 'src/RunicCommandLine.Processes/RunicCommandLine.Processes.csproj')
)
...
<configuration>
  ...
  <packageSourceMapping>
    <packageSource key="owned-command-line-feed">
      <package pattern="RunicCommandLine" />
      <package pattern="RunicCommandLine.*" />
    </packageSource>
...
try {
    ...
    $nativeExecutableName = if ($RuntimeIdentifier.StartsWith('win-', [StringComparison]::OrdinalIgnoreCase)) {
        'RunicCommandLine.PackageConsumer.exe'
    } else {
        'RunicCommandLine.PackageConsumer'
    }
