...
namespace RunicCommandLine.AotSmoke;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (result.State != ProcessState.Exited || result.ExitCode != 0)
        {
            CommandFault fault = result.Fault ??
                new CommandFault("RCLI_SMOKE_CHILD_FAILED", "The bounded child process failed.");
...
{
    internal const string PayloadIdentity = "runic.commandline.smoke/1";
