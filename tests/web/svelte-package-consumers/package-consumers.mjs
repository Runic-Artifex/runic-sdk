...
try {
  ...
  assert.equal(svelteManifest.peerDependencies["@runic-artifex/vite-plugin-runic"], viteManifest.version);
  ...
  await writeFile(
    ...
    [
      ...
      'if (!resource.disposed || runicToolkitSpaPageOptions.ssr !== false || typeof defineApplicationBridgeContract !== "function" || localizationStressCases.length !== 3 || pluralStressCounts.at(-1) !== 1000 || visualAccessibilityStressScenarios.length !== 3) throw new Error("package boundary failed");',
    ].join("\n"),
    "utf8",
  );
