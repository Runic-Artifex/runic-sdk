...
for (const [host, profile] of [["desktop", "default"], ["desktop", "minimal"], ["cswebui", "default"]]) {
  const name = `${host}-${profile}`;
  const project = copyApplication(name);
