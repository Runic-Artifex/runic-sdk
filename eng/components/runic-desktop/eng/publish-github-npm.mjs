...
function manifest(tarball) {
  ...
  if (result.status !== 0) {
    throw new Error(`cannot read ${tarball}: ${result.stderr}`);
  }
...
async function publishedBytes(name, version, token) {
  ...
  if (!metadata.ok) {
    throw new Error(`registry metadata request failed: ${metadata.status}`);
  }
...
const [, , tarball] = process.argv;
const token = process.env.NODE_AUTH_TOKEN;
if (!tarball || !token) {
  throw new Error(
    "usage: NODE_AUTH_TOKEN=... node eng/publish-github-npm.mjs <tarball>",
  );
}

const packageManifest = manifest(tarball);
const localBytes = fs.readFileSync(tarball);
const existing = await publishedBytes(
...
if (existing) {
  if (sha256(existing) !== sha256(localBytes)) {
    throw new Error(
      `immutable coordinate collision for ${packageManifest.name}@${packageManifest.version}`,
    );
  }
  ...
  const result = spawnSync(
    "bun",
    ["publish", "--registry", registry, "--tag", "ci", "--access", "restricted", tarball],
    {
      encoding: "utf8",
  ...
  if (result.status !== 0) {
    throw new Error(`publish failed: ${result.stdout}${result.stderr}`);
  }
