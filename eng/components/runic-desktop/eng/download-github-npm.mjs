...
const packageName = "@runic-artifex/desktop";

const [, , outputDirectory, version] = process.argv;
const token = process.env.NODE_AUTH_TOKEN;
if (!outputDirectory || !version || !token) {
  throw new Error(
    "usage: NODE_AUTH_TOKEN=... node eng/download-github-npm.mjs <directory> <version>",
  );
}
...
if (!metadataResponse.ok) {
  throw new Error(`registry metadata request failed: ${metadataResponse.status}`);
}
...
fs.writeFileSync(
  path.join(outputDirectory, `runic-artifex-desktop-${version}.tgz`),
  tarball,
);
