...
The workflow verifies the downloaded checksum before publication. Retries skip
an existing package only when npm's recorded SHA-512 integrity matches the
verified tarball. Prerelease versions use the `preview` dist-tag; stable versions
use `latest`.
