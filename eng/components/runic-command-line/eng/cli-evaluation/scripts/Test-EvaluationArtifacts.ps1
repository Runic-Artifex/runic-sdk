...
if ($manifest.schemaVersion -ne 'runic.commandline.evaluation/1') { throw 'Unexpected evaluation schema.' }
if ($manifest.protocolIdentity -ne 'runic.commandline/1') { throw 'Protocol identity drift.' }
if ($manifest.outputEnvironmentVariable -ne 'RUNIC_COMMANDLINE_OUTPUT') { throw 'Output-variable identity drift.' }
