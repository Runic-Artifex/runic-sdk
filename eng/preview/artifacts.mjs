...
export function authority(workspace) {
  ...
  assert.equal(workspace.nuget.length, 27); assert.equal(workspace.npm.length, 8);
  const entries = ['nuget', 'npm'].flatMap(registry => workspace[registry].map(p => ({ registry, name: p.name })));
  assert.equal(new Set(entries.map(p => p.name)).size, 35, 'Duplicate inventory identity');
  return entries;
}
