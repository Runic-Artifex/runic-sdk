...
These tools are optional for lifecycle/performance changes and regression
investigations. They are not publication prerequisites. The current release
policy is [../release/README.md](../release/README.md).
...
`7200000` when a two-hour investigation is useful. Choose the workload and
duration for the problem being investigated.
...
The driver samples the host and native/browser processes and reports resource,
operation, memory and shutdown failures. Its strict thresholds are diagnostic
signals for investigation. Keep the measured result unchanged and describe any
known limitation in an issue; there is no release waiver/receipt mechanism.

The package footprint runner removes its temporary consumers and keeps useful
reports under `artifacts/host-footprint`.

## Preparing comparable CI builds
