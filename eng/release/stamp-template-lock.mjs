// Packaging only: update disposable staged locks from finalized npm archives.
// Verification must never repair a shipped integrity mismatch.
...
if (!lockPath || archives.length === 0) {
  throw new Error("Usage: stamp-template-lock.mjs <staged-lockfile> <npm-archive>...");
}
...
for (const name of declared) {
  ...
  entry.resolved = `https://registry.npmjs.org/${name}/-/${name.slice(name.lastIndexOf("/") + 1)}-${candidate.version}.tgz`;
