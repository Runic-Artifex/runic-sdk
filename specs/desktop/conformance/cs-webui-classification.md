...
| Embedded HTML, explicit files/folders, MIME selection, redirects, and bridge route priority | Retained compatibility behavior | Behavioral tests remain differential evidence behind the public API. |
...
| 32-bit WebUI packet token as the only browser admission check | Intentional divergence | Runic Desktop uses canonical origin validation and a 256-bit surface-scoped credential before the compatibility token exchange. |
...
| Exact WebUI public type names and numeric C enum identity | No longer applicable | No public Desktop type contains `WebUi`; the internal profile carries only required wire behavior. |
