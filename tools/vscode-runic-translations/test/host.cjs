...
exports.run = async function () {
  ...
  const activated = JSON.parse(config.getText()); activated.executionProfile = 'rmf2-execution-v2';
  const activation = new vscode.WorkspaceEdit();
  activation.replace(config.uri, new vscode.Range(config.positionAt(0), config.positionAt(config.getText().length)), JSON.stringify(activated, null, 2));
  assert.equal(await vscode.workspace.applyEdit(activation), true, 'Could not activate execution-v2 through the synchronized manifest buffer');
  ...
  assert.equal(nextPreview.ast.astVersion, 5, 'Unsaved runic.json was not used to validate the renamed execution-v2 contract');
