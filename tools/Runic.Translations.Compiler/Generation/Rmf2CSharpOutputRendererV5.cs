...
internal static class Rmf2CSharpOutputRendererV5
{
    ...
    internal static TranslationGeneratedOutput RenderCatalogData(Rmf2ProjectV5 project)
    {
        ...
        V5Table table = V5Table.Create(project);
    ...
    private static void WriteDefinitions(GenerationWriter writer, V5Table table, string suffix)
    {
        writer.Line("new global::Runic.Translations.CompiledTranslationDefinition[]"); writer.Line("{"); writer.Indent();
        foreach (V5Definition definition in table.Definitions)
        {
    ...
    private static void WriteLocales(GenerationWriter writer, Rmf2ProjectV5 project, V5Table table, string suffix)
    {
    ...
    private static void WritePackContractFactory(GenerationWriter writer, Rmf2ProjectV5 project, V5Table table)
    {
        ...
        foreach (Rmf2LocaleV5 locale in project.Locales.OrderBy(item => item.Tag, StringComparer.Ordinal))
        {
            ...
            foreach (V5Definition definition in table.Definitions.Where(item => item.Canonical || resolved.Contains(item.Contract.Key)).OrderBy(item => item.Contract.Key, StringComparer.Ordinal))
            {
    ...
    private sealed record V5Definition(int Id, Rmf2MessageContractV5 Contract, bool Canonical);
    private sealed class V5Table
    {
        private readonly Dictionary<string, V5Definition> _byKey;
        private V5Table(IReadOnlyList<V5Definition> definitions) { Definitions = definitions; _byKey = definitions.ToDictionary(item => item.Contract.Key, StringComparer.Ordinal); }
        internal IReadOnlyList<V5Definition> Definitions { get; }
        internal int Id(string key) => _byKey[key].Id;
        internal Rmf2MessageContractV5 Contract(string key) => _byKey[key].Contract;
        internal static V5Table Create(Rmf2ProjectV5 project)
        {
            var result = new List<V5Definition>();
