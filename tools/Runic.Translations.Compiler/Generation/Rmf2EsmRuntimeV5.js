...
function validateMarkupNode(node,symbols,requirements,counts,stack) {
  ...
  for(const option of node.options){const memberError=closedMemberError(option,["name","value"]);if(memberError)return memberError==="unknown-member"?memberError:"argument-contract-mismatch";const valueError=validateValue(option.value);if(valueError)return valueError==="unknown-member"?valueError:"argument-contract-mismatch";if(!validName(option.name)||Object.hasOwn(values,option.name))return "argument-contract-mismatch";values[option.name]=option.value;}
...
function validateAnnotations(values,invalid) { if (!Array.isArray(values) || values.length > 256) return invalid; const names = new Set();for(const annotation of values){const memberError=closedMemberError(annotation,["name","value"],["name"]);if(memberError)return memberError==="unknown-member"?memberError:invalid;if(!validName(annotation.name,true)||names.has(annotation.name))return invalid;names.add(annotation.name);if(Object.hasOwn(annotation,"value")){if(annotation.value===undefined)return invalid;const valueError=validateValue(annotation.value);if(valueError)return valueError==="unknown-member"?valueError:invalid;if(["input","local"].includes(annotation.value.kind))return invalid;}}return null; }
function validateValue(value) { if (!isRecord(value)) return "malformed-pattern";const numeric=value.kind==="number-literal";const memberError=closedMemberError(value,["kind","value",...(numeric?["canonical"]:[]) ]);if(memberError)return memberError;if(!["input","local","string-literal","number-literal"].includes(value.kind)||typeof value.value!=="string"||numeric&&typeof value.canonical!=="string")return "malformed-pattern";if(["input","local"].includes(value.kind)&&!validName(value.value))return "malformed-pattern";if(numeric){try{if(decimalCanonical(parseDecimal(value.value))!==value.canonical)return "malformed-pattern";}catch{return "malformed-pattern";}}return null; }
