...
try {
    ...
    $receipt = @{sessionId=[Diagnostics.Process]::GetCurrentProcess().SessionId;windowName='Visual Studio RMF2';snapshotName='preview, validation, locale, unsaved restart';ideProcessId=$ideId;serverBefore=$before;serverAfter=$after.ProcessId;checks=@('registered preview command','native inert rich content','invalid-number recovery','locale selection','registered restart command','unsaved buffer after restart')}
}
finally {
